...
export async function getAllBreakfast(language: LangType): Promise<MealType[]> {
    await later(1000);

    const ranges: string[] = ["breakfast!A4:D1001"];

    switch (language) {
        case "en":
            ranges.push("breakfast!F4:F1001");
            break;
        case "az":
            ranges.push("breakfast!G4:G1001");
            break;
        case "ru":
            ranges.push("breakfast!H4:H1001");
            break;
    }
...
export async function getBreakfastById(id: string, language: LangType): Promise<MealType | undefined> {
    const meals = await getAllBreakfast(language);

    return meals.find((m) => m.id === id);
}

export async function getBreakfastByCategory(categoryId: string, language: LangType): Promise<MealType[]> {
    const meals = await getAllBreakfast(language);
